...
export function useChat(room: Room, me: { identity: string; name: string; seed: string }, active: boolean) {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [unread, setUnread] = useState(0);

  useEffect(() => {
    const onData = (payload: Uint8Array, participant?: Participant, _kind?: number, topic?: string) => {
  ...
  const send = useCallback(
    (raw: string) => {
      ...
      room.localParticipant
        .publishData(encodePayload(text, ts), {
          reliable: true,
          topic: CHAT_TOPIC,
        })
