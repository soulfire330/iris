    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="w-login max-w-full space-y-6">
        ...
        {rooms.length === 1 ? (
          ...
          <h2 className="text-xl font-medium tracking-title">{rooms[0].display}</h2>
        ) : (
          <>
            <h2 className="text-xl font-medium tracking-title">Куда заходим</h2>
            <div className="space-y-1.5">
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-sm">{r.display}</span>
                      <span className="flex items-center gap-3 font-mono text-xs tracking-mono text-neutral-500 uppercase">
                        {r.participants.length > 0 ? (
                          <span>{r.participants.length} в комнате</span>
                        ) : (
                    ...
                    {r.participants.length > 0 && (
                      <span className="flex flex-none items-center">
                        ...
                        {r.participants.length > 3 && (
                          <span
                            className={cn(
                              "flex size-5.5 items-center justify-center rounded-full border bg-neutral-800 text-xs text-neutral-300",
                              active ? "border-card" : "border-background",
                              "-ml-0.5",
                            )}
                          >
              ...
              {!expanded && hiddenRooms > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 pl-0 text-sm text-neutral-500 transition-colors hover:text-neutral-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
                >
                  <CaretDown size={15} />
        ...
        <form onSubmit={submit} className="space-y-6">
          ...
          <div className={cn("space-y-3", dim && "pointer-events-none opacity-45")}>
            <div className="space-y-1.5">
              ...
              />
              {loginAlreadyInRoom && (
                <p className="text-xs text-warn">Этот логин уже в комнате — второй сеанс с тем же именем.</p>
              )}
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                required
              />
              {badLogin && <p className="text-xs text-danger-300">Логин или пароль не подошли.</p>}
            </div>
          </div>
          ...
          {offline && (
            <div className="flex items-center gap-2 rounded-sm bg-background px-3 py-2 shadow-sm">
              ...
              <span className="text-xs text-neutral-500">Сервер не отвечает, пробуем снова…</span>
            </div>
          )}
          <Button
            type="submit"
          ...
          >
            <span className="truncate">{busy ? "Входим…" : `Войти в «${selected?.display ?? ""}»`}</span>
