  ...
  return (
    <div className="flex h-screen w-screen flex-col overflow-hidden bg-background text-foreground">
      <RoomHeader
        ...
        panelBtnClass={layout === "stage" || layout === "summaries" ? "xl:hidden" : "lg:hidden"}
        screenLabel={
          // Тег в шапке: «Экран», если кто-то демонстрирует (независимо от того,
          // чей поток на крупном плане); иначе — развёрнутая камера.
          sharer ? "Экран" : stage && stageMember ? `камера ${stageMember.name}` : undefined
        }
      />
      {(error || actionError) && (
        <div className="flex-none border-b border-border bg-card px-6 py-1.5 font-mono text-xs text-warn">
          {error || actionError}
        </div>
      )}
      ...
      <div
        className={cn(
          ...
          layout === "stage" || layout === "summaries"
            ? "[grid-template-columns:1fr] lg:[grid-template-columns:1fr_208px] xl:[grid-template-columns:1fr_208px_300px]"
            : "[grid-template-columns:1fr] lg:[grid-template-columns:1fr_300px]",
        )}
      >
        {layout === "summaries" ? (
          <SummariesView
          ...
          <div className="flex min-h-0 min-w-0 flex-col gap-4 p-6">
            <div
              ref={gridRef}
              className={cn(
                "@container flex min-h-0 flex-1 flex-wrap content-center justify-center gap-3",
