# Виртуальный офис

Корпоративный голосовой хаб команды: одна постоянная комната, сотрудники описаны в config.yaml, записи встреч по кнопке. Разворачивается через docker compose.

## Language

**Сотрудник** (Employee):
Пользователь хаба, описанный в config.yaml; идентичность — login + password.
_Avoid_: юзер, аккаунт

**Участник** (Participant):
Сотрудник, подключённый к комнате в данный момент (термин LiveKit для presence).
_Avoid_: юзер

**Гость** (Guest):
Человек, вошедший по инвайт-ссылке: нет логина в config.yaml, вход без пароля.
Идентичность — `inv-<токен>` (уникален на инвайт), роль `guest` в metadata
токена: не управляет записью и не создаёт инвайты.
_Avoid_: временный сотрудник

**Комната** (Room):
Единственная постоянная комната команды; существует, пока в ней есть участники.
_Avoid_: конференция, звонок

**Запись** (Recording):
Аудио-файл отрезка жизни комнаты, начатый/остановленный кнопкой в интерфейсе.
_Avoid_: рекординг

**Сводка** (Summary):
Краткий пересказ записи (тема, решения, договорённости), собранный Секретарём из транскрипта.
_Avoid_: саммари, протокол

**Секретарь** (Secretary):
Воркер AI-сводок (`server/cmd/secretary`): распознаёт речь записи (STT) и составляет Сводку (LLM); настройка — `.env`.
_Avoid_: AI-воркер, бот

## Frontend: Tailwind CSS (web/)

- Классы сортирует `prettier-plugin-tailwindcss` (prettier — dev-зависимость, формат через `bunx prettier --write src/`).
- Динамические классы — только через `cn()` (`src/lib/utils.ts`, clsx + tailwind-merge). Никаких `px-4 ${isActive ? ... }`.
- Произвольные значения `[...]`: не чаще 1–2 использований. Повторы — токен в `@theme inline` в `src/index.css` (`--size-*`, `--text-*`, `--shadow-*`, `--tracking-*`, `--background-image-*`). Сложные расчёты (cqw, clamp) допустимы: см. `avatarSize` в ParticipantTile.
- Mobile-first: только `sm:`/`md:`/`lg:`/`xl:`/`2xl:` (min-width). `max-*` — запрещено, в коде их нет.
- Шорткаты: `size-*` вместо пар `w-* h-*`, `inset-0`, `aspect-video`.
- Элемент зависит от родителя — `@container` + cqw; от экрана — viewport-префиксы. Не смешивать в одном элементе.
- Повторяющиеся блоки из 3+ мест — компонент, а не `@apply` (исключение — базовые стили тегов в `@layer base`).
