...
| AI-сводки | Кнопка AI видна при идущей записи и заказывает сводку: флаг `summary` в sidecar записи (ADR-0004). `server.auto_summary: true` заказывает сводку каждой новой записи со старта — без кнопки. Отдельный воркер-секретарь (`server/cmd/secretary`) разбирает такие записи STT → LLM и кладёт `{имя}.summary.json`; секреты — `.env`, не config.yaml. Сводка — в табе «Сводки». |
...
- [x] Флаг `server.auto_summary` (config.yaml): сводка заказывается каждой новой записи со старта — кнопка AI не нужна. Только новые записи, без backfill архива.
