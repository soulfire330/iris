...
Снимок комнаты `GET /api/room` несёт `avatar` (raw SVG-строка) для каждого
участника — плитки-заглушки до коннекта не ходят в `/api/avatar`; после
коннекта новые участники приходят через LiveKit (identity + seed в metadata)
и рисуются через `/api/avatar/{login}?v=seed` — эндпоинт остаётся
публичным, `<img>` не умеет заголовки. Ответы API сжимаются gzip/brotli (httpcompression, по Accept-Encoding).
...
порядок как в конфиге). Список живой: счётчик участников, аватары и метка
записи обновляются при каждом опросе (экран входа опрашивает раз в 5 секунд):
...
| `participants` | все участники: `identity` — логин (для проверки «логин уже в комнате»), `name` — имя, счётчик людей — `len(participants)`. Первые три участника дополнительно несут `avatar` — raw SVG-строка: экран входа рисует аватар без обращения к `/api/avatar` |

Сжатие: все ответы `/api/*` отдаются gzip/brotli (httpcompression, по
`Accept-Encoding`), mp4 — без сжатия (Range-запросы). Raw SVG ужимается
в ~3 раза — base64 не нужен.
