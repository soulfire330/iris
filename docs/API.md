...
публичными остаются `login`, `healthz`, `rooms`, аватары и инвайты
(страница гостя и вход — до авторизации).
...
| `participants` | имена присутствовавших на стопе (пустой при самообрыве); гости — именем из инвайта |
...
## Инвайты (гостевой вход)

Вход без логина и пароля: сотрудник создаёт ссылку, гость по ней попадает на
страницу-сводку (имя, комната, участники, запись) и входит одной кнопкой.

| Эндпоинт | Доступ | Что делает |
|---|---|---|
| `POST /api/invite?room=` `{name, ttl_sec}` | токен, сотрудник | создать инвайт (`ttl_sec`: 60–2592000, потолок 30 дней; бесконечных нет) |
| `GET /api/invites?room=` | токен, сотрудник | живые инвайты комнаты (все, не только свои) |
| `DELETE /api/invite/{token}?room=` | токен, сотрудник | отозвать (будущие входы перестают работать; сидящие не вылетают) |
| `GET /api/invite/{token}` | публичный | страница гостя: имя, аватар, комната, участники, `recording` |
| `POST /api/invite/{token}/join` | публичный | вход: LiveKit-токен с ролью `guest`; многоразовый до TTL/отзыва |

Инвайты хранятся в `data_dir/invites.json` (переживают рестарт, отзыв возможен),
токен — 256 бит случайности (bearer-креденциал ссылки). Роль `guest` не
управляет записью и не создаёт инвайты — проверка на сервере, не только в UI.
Ошибки: `404` — не найден/отозван, `410` — истёк.
